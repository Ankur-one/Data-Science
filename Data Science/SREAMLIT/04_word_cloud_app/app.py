...
if uploaded_file:
    ...
    # Sidebar: Checkbox and stopword controls
    use_standard_stopwords = st.sidebar.checkbox("Use standard stopwords?", True)
    top_words = word_count['Word'].head(50).tolist()
    additional_stopwords = st.sidebar.multiselect("Choose additional stopwords:", sorted(top_words))
    custom_stopwords = st.sidebar.text_input("Add custom stopwords (comma-separated):")
    custom_stopwords_list = [word.strip() for word in custom_stopwords.split(",") if word.strip()]
    selected_stopwords = set(additional_stopwords) | set(custom_stopwords_list)

    if use_standard_stopwords:
        all_stopwords = STOPWORDS.union(selected_stopwords)
    else:
        all_stopwords = selected_stopwords
    ...
    st.sidebar.markdown("Created by: [Ankur Kumar](https://github.com/Ankur-one)")
    st.sidebar.markdown("Contact: [Email](mailto:2005ankurkumar@gmail.com)")
